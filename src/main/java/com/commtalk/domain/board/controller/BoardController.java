...
public class BoardController {
    ...
    @Operation(summary = "핀고정 게시판 추가 및 삭제")
    @PostMapping(path = "/pinned")
    @Transactional(rollbackFor = Exception.class)
    public ResponseEntity<List<BoardDTO>> updatePinnedBoards(@RequestBody PinAndUnpinBoardDTO updateDto, HttpServletRequest request) {
        Long memberId = jwtAuthenticationProvider.getMemberId(request);
        boardSvc.unpinBoards(memberId, updateDto.getUnpinBoardIds()); // 게시판 핀고정 해제
        boardSvc.pinBoards(memberId, updateDto.getPinBoardIds()); // 게시판 핀고정
    ...
    public ResponseEntity<ResponseDTO<String>> reorderPinnedBoard(@RequestBody List<BoardDTO> boardDtoList, HttpServletRequest request) {
        Long memberId = jwtAuthenticationProvider.getMemberId(request);
